...
public interface UserRepository extends JpaRepository<UserDB, Integer> {
    ...
    List<UserDB> findByStatusAndNicknameContaining(String status, String nickname, Pageable paging);
...
@Query("select u " +
        "from UserDB u " +
        "inner join UserAreaDB ua on u.id = ua.user.id " +
        "and u.status = 'ACTIVATE' and ua.orders = 1 " +
        "where u.id <> ?1 and ua.area.id = ?2")
    List<UserDB> findAreaStar(int userId, int areaId, Pageable paging);
}